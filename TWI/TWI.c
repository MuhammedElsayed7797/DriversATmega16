...
void TWI_Init (void)
{
	...
	/* initialize bit rate */
	*(TWAR_REG) = ( TWI_DEVICE_ADDRESS << 1 ) ;

	*(TWBR_REG) = TWI_BIT_RATE ;

	/* initialize prescaler */
	TWSR_REG_BIT->Bit_0 = TWI_PRESCALER & 0b01 ;
	TWSR_REG_BIT->Bit_1 = (TWI_PRESCALER >> 1 )  ;
	...
	*(TWCR_REG) = 0b10000101 ;
	...
	DDRC_REG_BIT->Bit_0 =  FALSE ;
	DDRC_REG_BIT->Bit_1 =  FALSE ;
	PORTC_REG_BIT->Bit_0 =  TRUE ;
	PORTC_REG_BIT->Bit_1 =  TRUE ;
...
StatusType TWI_MT ( uint8 * TransmittedData, uint8  SlaveAddress, uint8 DataSize )
{
	...
	if ( FALSE == ( TWI_ModeBusy.MT_Mode | TWI_ModeBusy.MR_Mode ) )
	{
		...
		TWCR_REG_BIT->Bit_4 = FALSE ;

		/* to initiate start */
		TWCR_REG_BIT->Bit_5 = TRUE ;
...
ISR ( TWI_ISR )
{
	...
	TWI_CurrentState = *(TWSR_REG) & 0xF8 ;
	...
	switch ( TWI_CurrentState )
	{
		...
		case (TWI_TRANSMIT_START) :
		{
			...
			*(TWDR_REG) =  TWI_Internal_SLA_R_W;

			TWCR_REG_BIT->Bit_5 = FALSE ;
		...
		case (TWI_TRANSMIT_SLA_W_ACK) :
		{
			...
			*(TWDR_REG) =  TWI_InternalMT_DataAddress[TWI_Internal_DataIndex] ;
		...
		case (TWI_M_RECEIVE_DATA_ACK) :
		{
			...
			TWI_InternalMR_DataAddress[TWI_Internal_DataIndex] = *(TWDR_REG) ;
			TWI_Internal_DataIndex ++ ;
		...
		case (TWI_M_RECEIVE_DATA_NACK) :
		{
			...
			TWCR_REG_BIT->Bit_6 = TWI_SlaveFlag ;
			TWCR_REG_BIT->Bit_4 = TRUE ;
		...
		case (TWI_S_RECEIVE_DATA_ACK) :
		{
			...
			TWI_InternalSR_DataAddress[TWI_Internal_DataIndex] = *(TWDR_REG) ;
			TWI_Internal_DataIndex ++ ;
		...
		case (TWI_RECEIVE_SLA_R_ACK) :
		{
			...
			*(TWDR_REG) =  TWI_InternalST_DataAddress[TWI_Internal_DataIndex] ;
